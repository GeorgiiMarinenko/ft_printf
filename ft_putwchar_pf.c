...
/*   Updated: 2020/11/25 18:53:24 by aarlena          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_printf.h"

static void	ft_putwchar_2(wchar_t wc, t_args *f)
{
	char wstr[4];
	...
	wstr[2] = (((wc >> 6) & 63) + 128);
	wstr[3] = ((wc & 63) + 128);
	f->len += write(f->file_descr, wstr, 4);
}
...
void		ft_putwchar_pf(wchar_t wc, t_args *f)
{
	...
	if ((wc >= 0 && wc <= 127) || (wc >= 0 && wc <= 255 && MB_CUR_MAX == 1))
		f->len += write(f->file_descr, &wc, 1);
	else if (wc >= 128 && wc <= 2047 && MB_CUR_MAX >= 2)
	{
		wstr[0] = ((wc >> 6) + 192);
		wstr[1] = ((wc & 63) + 128);
		f->len += write(f->file_descr, wstr, 2);
	}
	else if (wc >= 2048 && wc <= 65535 && MB_CUR_MAX >= 3)
	{
		...
		wstr[1] = (((wc >> 6) & 63) + 128);
		wstr[2] = ((wc & 63) + 128);
		f->len += write(f->file_descr, wstr, 3);
	}
